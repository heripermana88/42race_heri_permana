...
const StravaProcessorAdapter = function () {
  const adapter = {
    auth: function (type, code) {
      const url = `${process.env.STRAVA_URL}${paths.auth}`;
      const params = {
      ...
      switch (type) {
        case 'EXCHANGE_TOKEN':
          return handleResponse('POST', url + '/token', { args: params });
          break;
        case 'DEAUTHORIZE':
          return handleResponse('POST', url + '/deauthorize', { access_token: code });
          break;
      }
    },
